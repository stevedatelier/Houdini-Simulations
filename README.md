...
* [<small><sup>Speed-up Render time / Dicing: shading quality</sup></small>](#Dicing)
...
These controls are on the Limits tab of the Mantra render node. I would leave these parameters to their default values except for diffuse limit and volume limit (if there actually volumes are in your scene). Raise diffuse limit to 4. You can also raise the color limit to a number like 10 or more, but keep in mind that it could increase render time.
